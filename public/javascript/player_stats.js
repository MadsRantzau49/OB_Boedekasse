...
async function check_number_of_players_found(search_player) {
...
async function search_player_function(search_player){
    let matches_list = [];
    ...
    let dataPlayer = await fetchData("player-finance");
    const jsonDisplayDiv = document.getElementById("player_finance_table");
    dataPlayer.payingPlayers.forEach(player => {
        if(player.dbu_name.toLowerCase().includes(search_player.value.toLowerCase())){
